...
static char *get_private_key_password_from_keychain(const char *certificate_file, const char *private_key_file)
{
        ...
        char            *password        = NULL;
        uint8_t         *cert_file_buf   = NULL;

        DEBUG2("rlm_eap_tls: retrieving password from keychain for private key file %s", private_key_file);

        /* Read the certificate data. */
        struct stat sb;
        if (stat(certificate_file, &sb) != 0) {
                radlog(L_ERR, "rlm_eap_tls: Certificate file not found - %s", certificate_file);
                goto out;
        }

        cert_file_buf = malloc(sb.st_size);
        if (!cert_file_buf) {
                radlog(L_ERR, "rlm_eap_tls: Unable to allocate %lld bytes to read certificate file - %s", sb.st_size, certificate_file);
                goto out;
        }

        fd = open(certificate_file, O_RDONLY);
        if (fd < 0) {
                radlog(L_ERR, "rlm_eap_tls: Unable to open certificate file - %s", certificate_file);
                goto out;
        }

        ssize_t bytes_read = read(fd, cert_file_buf, sb.st_size);
        if (bytes_read < 0) {
                radlog(L_ERR, "rlm_eap_tls: Unable to read certificate file - %s: %s", certificate_file, strerror(errno));
                goto out;
        }

        if (bytes_read != sb.st_size) {
                radlog(L_ERR, "rlm_eap_tls: Unable to read entire certificate file - %s: read %zd bytes, expected to read %lld bytes", certificate_file, bytes_read, sb.st_size);
                goto out;
        }
        ...
        if (sec_items == NULL || CFArrayGetCount(sec_items) != 1) {
                radlog(L_ERR, "rlm_eap_tls: Unable to convert certificate CFData into SecItems");
                goto out;
        }
	...
	if (conf->private_key_password) {
		...
		if (strncmp(conf->private_key_password, special_string, strlen(special_string)) == 0)
		{
			free(conf->private_key_password);
                        ...
                        if (!conf->private_key_password) {
                                radlog(L_ERR, "rlm_eap_tls: Error reading private key password from keychain");
                                return NULL;
                        }
		}
#endif
